...
FOLDER_ID = "1UK_F280M9tNVW9_amB27VHOUMEPCf6ze"
...
def calculate_key_items_threshold(tolerable_error, percentage_range):
    ...
    selected_percentage = st.number_input(
        ...
        value=float(percentage_range[0]),
        step=0.1
    )
...
def calculate_additional_sample_size(population_data, account_nature, sample_size):
    ...
    additional_sample_size = max(1, int(sample_size * 0.1))
    if len(other_side_data) < additional_sample_size:
        additional_sample_size = len(other_side_data)
...
def main():
    ...
    uploaded_file = st.file_uploader("Upload Population Data (CSV or Excel)", type=["csv", "xlsx", "xls"])
    if uploaded_file:
        try:
            if uploaded_file.name.endswith('.csv'):
                population_data = pd.read_csv(uploaded_file)
            elif uploaded_file.name.endswith(('.xlsx', '.xls')):
                population_data = pd.read_excel(uploaded_file)
            else:
            ...
            if not item_number_col or not item_value_col:
                ...
                st.sidebar.write(f"**Total Population Value (Auto-calculated):** {total_population_value:,.2f}")
                ...
                st.write(f"Tolerable Error: {tolerable_error:,.2f}")
                ...
                if negative_testing_col:
                    ...
                    additional_sample = pd.DataFrame()
